...
After initial setup, run `python main.py` to start the server. Upon starting the server, any documents in the docs folder will be embedded and indexed with Faiss. Your local llama3 instance will use __*ONLY*__ the information contained in your documents for context. Currently supported document types include .txt, .md, and .pdf. PDFs must be already machine readable; future work will incorporate OCR to create machine readable context from PDFs/images on the fly.
